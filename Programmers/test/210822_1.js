...
따라서 [true, false]를 리턴합니다.
    ...
    함수
        ...
        logon 함수
            아이디 비번으로 체크하고 로그인 상태로 바꿈
        
        add 함수
            로그인 상태 체크하고 장바구니에 담긴 음식 체크
        
        order 함수
            장바구니 상태 체크하고 있으면 주문 후 장바구니 비움
            장바구니에 담긴 음식 없으면 제거
*/
function solution(infos, actions) {
    ...
    answer = actions.map( e => {
        if(e.substr(0,5).indexOf("LOGIN") !=-1){
            return checkLogon(e)
        }
        else if(e.substr(0,3).indexOf("ADD") !=-1){
            if(!logonState)
                return false
            return checkAdd(e)
        }
        else if(e.substr(0,5).indexOf("ORDER") !=-1){
            if(!orderState)
                return false
            return checkOrder(e)
        }
...
var infos = ["kim password", "lee abc"]
var actions = [
    "ADD 30", 
    "LOGIN kim abc", 
    "LOGIN lee password", 
