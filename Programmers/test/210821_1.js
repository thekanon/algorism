...
잘못된 호수를 입력한 경우
잘못된 비밀번호를 입력한 경우
본 문제에서는 XX아파트에서 공동현관 출입을 위해 입력한 내용이 한 줄로 된 문자열 형태로 주어질 때, 
올바르게 비밀번호를 입력하여 공동현관문을 연 횟수는 총 몇 번인지 구하면 됩니다. 
...
각 세대에서 사용하는 비밀번호가 담긴 2차원 배열 passwords, 공동현관문에 입력된 내용이 담긴 문자열 s가 매개변수로 주어질 때, 
공동현관문은 총 몇 번 열렸는지 return 하도록 solution 함수를 완성해주세요.
...
s를 입력받으면 split으로 잘라서 별도의 변수에 넣고
passwords랑 비교하면 되지 않을까
-> 잘못생각함 호수 먼저 검증하고 그 뒤에 내용을 생각해야함

for문돌면서

호수테스트 true시 비번 테스트 true시 count
반복
...
function solution(passwords, s) {
    ...
    var roomTest = (num) => {
        ...
        for(var i=0;i<passwords.length;i++){
            if(passwords[i][0]==num)
                result = i
        }
        return result
    }
    var passTest = (t,i) => {
        return passwords[t][1]==i ? true : false
        
    }
    sArr = s.split("#");

    for(var i=0;i< sArr.length-1;i++){
        var t = roomTest(sArr[i]);
        if(t!=-1){
            i++
            answer = passTest(t,sArr[i]) ? answer+1 : answer
        }
    }

    return answer;
}
solution([[101,1234],[102,54321],[201,202],[202,1]]	,"101#1234#102#654321#51#203#201#202#1#")
solution([[101,9999],[102,1111]],	"201#9999#101#")	
solution([[101,9999],[102,1111]],	"101#9999#102#1111#101#9999#101#9999#"	)