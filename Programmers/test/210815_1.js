...
다른 기지국의 영역과 겹칠 수도 있기 때문에 계산이 복잡해질 수 있습니다. 
그래서 몬테카를로 기법을 사용하여 면적을 구하려고 합니다.

몬테카를로 방식으로 면적을 계산하는 방법은 다음과 같습니다.

주어진 기지국 영역을 모두 포함하는 가장 작은 직사각형을 찾습니다. 
이 직사각형의 네 변은 x축 또는 y축과 평행해야 합니다. 
직사각형의 왼쪽 변의 x좌표를 l, 오른쪽 변의 x좌표를 r, 아래쪽 변의 y좌표를 b, 위쪽 변의 y좌표를 t라고 합니다.
난수를 2개 생성(각각 x, y라고 함)하여 이 직사각형 안의 점을 나타내는 좌표로 변환합니다. 
이 문제에서는 좌표를 정수로만 표현합니다. 
직사각형 안에 위치를 특정하기 위해 좌표를 변환할 수도 있습니다. 
변환된 좌표는 (l + x % (r - l), b + y % (t - b))입니다. 
여기서 %는 나머지 연산으로, a%b는 a를 b로 나눈 나머지를 의미합니다. 
나머지 연산은 더하기 연산보다 우선순위가 높습니다.
점이 기지국 영역 내에 있는지 검사합니다. 
원 위에 존재하는 경우도 영역 내에 있는 것으로 간주합니다.
생성한 점들 중에서 기지국 영역 내부에 존재하는 점의 비율을 계산합니다. 
이 비율을 직사각형의 면적과 곱하여 기지국 영역의 면적을 구합니다. 
