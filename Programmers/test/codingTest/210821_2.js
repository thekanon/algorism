...
변수 생성과 변수 값 대입: 변수명=숫자 의 형태로 이루어져 있습니다. 
이 명령어를 실행하면, 변수를 생성하고 숫자를 변수에 대입합니다.
...
2	..a=4	첫 번째 블록에 포함되는 두 번째 블록이 생성됩니다. 두 번째 블록에 a변수가 생성되고 4의 값을 가집니다. 
이때, 1번째 명령어에서 생성된 변수와 다른 변수입니다. 즉, 서로 영향을 주지 않습니다.

3	..b=3	두 번째 블록입니다. b변수가 생성되고 3의 값을 가집니다.

4	..print a	두 번째 블록입니다. 변수 a를 출력합니다. 
이때 존재하는 a는 1번 명령어와, 2번 명령어에서 생성된 것 두 가지입니다. 
그중 가장 나중에 생성된 변수를 출력합니다. 따라서 a=4를 출력합니다.

5	.......a=6	두 번째 블록에 포함되는 세 번째 블록이 생성됩니다. 
세 번째 블록에 a변수가 생성되고 6의 값을 가집니다. 
이때, 생성된 a는 1번 명령어에서 생성된 a와 2번 명령어에서 생성된 a와는 다른 변수입니다. 
즉, 서로 영향을 주지 않습니다.

6	.......print a	세 번째 블록입니다. 변수 a를 출력합니다. 이때 존재하는 a는 세 개입니다. 
그중 가장 나중에 생성된 변수의 값을 출력합니다. 따라서 a=6을 출력합니다.

7	.......print b	세 번째 블록입니다. 변수 b를 출력합니다. 
이때 존재하는 b는 한 개입니다. 따라서 b=3을 출력합니다.

8	..print a	세 번째 블록이 사라졌습니다. 따라서 5번 명령어에서 생성한 a변수가 사라지게 됩니다. 
변수 a를 출력합니다. 이때 존재하는 a는 두 가지입니다. 
1, 2번 명령어에서 생성된 a만 존재하고, 5번 명령어에서 생성된 a는 블록이 끝나 삭제되었기 때문에 존재하지 않습니다. 
따라서 존재하는 a 중 가장 나중에 생성된 a=4를 출력합니다.
...
현재 블록의 상태를 판단하는 함수 choiceBlock
    현재 블록의 위치가 크면 push
    현재 블록의 위치가 작으면 pop
    현재 블록의 위치가 그대로면 유지

어떤 명령어인지 판단하는 함수 choiceFunc
    변수생성인지 변수출력인지
*/
function solution(code) {
    ...
    var addBlock = (blockNum) => {
        currentBlock = blockNum;
        var i = answer.push([])-1;
        answer[i][0]=blockNum;
    }
    var delBlock = (blockNum) => {
        for(;;){
            if(answer[answer.length-1][0]>blockNum){
                answer.pop();
            }else {
                break;
            }
    ...
    var printValue = (value) => {
        ...
        for(var i =answer.length-1;i>=0;i--){
            if(answer[i][1][value]){
                answerPrint.push(value+"="+answer[i][1][value]);
                errorFlag = false
                break;
            }
        }
        if(errorFlag)
            answerPrint.push("error")
    }
    var createValue = (value) => {
        ...
        answer[answer.length-1][1][key] = val
    }
    for(var i=0;i<code.length;i++){
        var blockNum = code[i].split(".").lastIndexOf("")+1 //현재 블록 .갯수
        var printFlag = code[i].split("print ") //출력할 값
        if(blockNum>currentBlock){  //현재 블록이 가장 크면 블록 생성
            addBlock(blockNum)
        }else if(blockNum<currentBlock){    //현재 블록이 작으면 블록 삭제
            delBlock(blockNum)
        }
        if(printFlag[1]){
            printValue(printFlag[1])
        }else {
            createValue(printFlag[0]);
        }
    }

    return answerPrint;
}
var code = ["a=3", "..a=4", "..b=3", "..print a", ".......a=6", ".......print a", ".......print b", "..print a", "....a=7", "....print a", "print a", "print b", "a=4", "print a", "...print a"]	
